...
// ✅ “Ngày diễn ra” = “Ngày tạo bài”
// -> Không cho nhập ngày nữa, tự lấy thời gian thực hiện tại khi tạo
type FestivalEvent = {
  id?: string;
  name: string;
  image: string;
  ...
  // Giữ lại để tương thích DB cũ (nếu đang có field date)
  date?: string;

  // Field chính dùng để hiển thị “Ngày tạo bài”
  created_at: string;
};
...
export default function FestivalManager() {
    ...
    fetch(API_URL)
      ...
      .then((data: any[]) => {
        const normalized: FestivalEvent[] = (Array.isArray(data) ? data : []).map((item) => {
          // ✅ Nếu DB cũ chưa có created_at, lấy từ date (hoặc tạo mới)
          ...
          return {
            ...
            created_at: created,
            date: created, // ✅ luôn sync date = created_at cho đúng “ngày tạo bài”
          };
        });
  ...
  const openModal = (item: FestivalEvent | null = null) => {
    if (item) {
      // đảm bảo sync
      const created = item.created_at || item.date || nowIso();
      setModalData({ ...item, created_at: created, date: created });
    } else {
      ...
      setModalData({
        ...
        image: "",
        description: "",
        created_at: created, // ✅ ngày tạo bài thực tế
        date: created,       // ✅ sync
      });
    }
    setIsModalOpen(true);
  ...
  const saveEvent = async () => {
    ...
    try {
      if (exists) {
        ...
        // ADD: bỏ id tạm, created_at = thời gian thực hiện tại
